...
把 push 放在最后是很巧妙的！在[《解决iOS项目文件合并.xcodeproj冲突》](https://blog.bingo.ren/11.html)中，用 xUnique 去解决问题，切入点就是 hooks，选择的是 pre-commit。这种情况不适合 pre-commit，而应该选择 pre-push。这样就可以用 pre-push 这个钩子来先调用 Shell 脚本。对我们而言，不需要敲什么别的命令，只要一句非常普通的 push 命令。
...
完整的 shell 脚本与 hook 已经放上 [GitHub-DKIpaBuilder](https://github.com/bingozb/DKIpaBuilder)，如果您有更好的编程笔记，欢迎 Issues。
